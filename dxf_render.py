...
def build_config(
    ...
    Notes:
        HATCH é ignorado (``HatchPolicy.IGNORE``). Hachuras são preenchimento
        que não carrega informação útil para PSCIP e ainda cobre o conteúdo —
        o código já as exclui em ``_no_fill_filter`` e ``_EXCLUDE_TYPES_DEFAULT``.
        Mas esses filtros só pegam HATCH no topo do modelspace; hachuras
        aninhadas dentro de INSERT (blocos) viram entidades virtuais na hora do
        render e escapam. Cada uma pode estourar o ``hatching_timeout`` de 30s
        do ezdxf e travar a resposta. IGNORE corta isso na raiz, para qualquer
        nível de aninhamento. ``hatching_timeout`` baixo é seguro extra.
    """
