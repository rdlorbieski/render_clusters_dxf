...
# Acima desta razão p90/p10 de alturas, o arquivo mistura duas escalas de texto
# (projeto + topografia/cotas) — não pode ser 'alta', pois a localização
# automática dos quadros fica comprometida.
_MIXED_SCALE_SPREAD = 20.0


def _detect_quality(
    info, positions: list | None = None, height_spread: float | None = None
) -> tuple[str, str]:
    """
    ...
    - mistura de escalas de texto (height_spread alto): rebaixa 'alta' → 'media'
    ...
    Args:
        height_spread: razão p90/p10 das alturas de texto (ver
            _text_height_spread). Quando alto, denuncia camada topográfica/cotas
            em escala diferente do projeto — impede o veredito 'alta'.
    """
    mixed_scale = height_spread is not None and height_spread >= _MIXED_SCALE_SPREAD
    ...
    # Mistura de escalas: projeto + topografia/cotas no mesmo arquivo. Impede o
    # veredito 'alta' — a localização automática dos quadros fica comprometida.
    if mixed_scale:
        return "media", (
            f"Arquivo com escalas de texto misturadas (dispersão de alturas "
            f"{height_spread:.0f}×): há uma camada topográfica/de cotas em escala "
            "muito diferente do projeto arquitetônico. Isso distorce a medição da "
            "escala do texto e pode prejudicar a localização automática dos quadros "
            "de RT e legendas — recomenda-se conferir os resultados."
        )
...
# Salto multiplicativo entre alturas consecutivas que denuncia DUAS escalas de
# texto no mesmo modelspace (ex.: projeto arquitetônico + camada topográfica de
# cotas/contornos com texto centenas de vezes maior). Acima disto, medir a
# altura sobre TODOS os textos envenena a escala do grid → detecção degenera.
_SCALE_GAP = 8.0


def _collect_text_heights(msp) -> list[float]:
    """Alturas (>0) de TODAS as entidades TEXT/MTEXT do modelspace, ordenadas.

    Percorre o modelspace inteiro (sem corte antecipado): a ordem das entidades
    não é aleatória e um corte cedo pode amostrar só uma das escalas — ex.: a
    camada topográfica costuma vir primeiro, escondendo o texto de corpo do
    projeto que vem depois.
    """
    heights: list[float] = []
    for e in msp:
        try:
...
def _lower_scale_cluster(sorted_heights: list[float]) -> list[float]:
    """Se as alturas se dividem em duas escalas, mantém só o agrupamento inferior.

    Procura o maior salto multiplicativo entre alturas consecutivas (ignorando
    os 5% extremos, que costumam ser ruído tipo altura 0.12 ou um título solto).
    Se o salto for grande (>= _SCALE_GAP), é sinal de conteúdo em duas escalas —
    fica com o grupo menor, que é o texto de corpo real do projeto.
    """
    n = len(sorted_heights)
    if n < 10:
        return sorted_heights
    lo, hi = max(int(n * 0.05), 1), int(n * 0.95)
    best_ratio, split = 1.0, None
    for i in range(lo, hi):
        prev = sorted_heights[i - 1]
        if prev <= 0:
            continue
        ratio = sorted_heights[i] / prev
        if ratio > best_ratio:
            best_ratio, split = ratio, i
    if split is not None and best_ratio >= _SCALE_GAP:
        return sorted_heights[:split]
    return sorted_heights


def _text_height_spread(msp) -> float:
    """Razão p90/p10 das alturas de texto — indicador de mistura de escalas.

    ~1–3 em arquivos saudáveis (títulos vs corpo). Centenas quando o arquivo
    empacota uma camada topográfica/cotas numa escala totalmente diferente do
    projeto. Retorna 1.0 se não houver amostra suficiente.
    """
    heights = _collect_text_heights(msp)
    if len(heights) < 10:
        return 1.0
    p10 = heights[int(len(heights) * 0.10)]
    p90 = heights[int(len(heights) * 0.90)]
    return (p90 / p10) if p10 > 0 else 1.0
...
def _text_height_dxf(msp, percentile: float = 0.20) -> float | None:
    ...
    Robusto a arquivos que misturam duas escalas de texto (ver
    _lower_scale_cluster): a escala grande (topografia/cotas) é descartada
    antes de tirar o percentil, senão o grid da detecção degenera.
    ...
    if len(heights) < 5:
        return None
    heights = _lower_scale_cluster(heights)
